...
def main():
    ...
    party_mean = raw[is_gen & ~R1 & ~R2 & ~R3].groupby("party")["score"].mean()
    ...
    with open(ANOMALY_LOG, "w", encoding="utf-8") as fh:
        ...
        for rkey, rlabel in LABELS.items():
            ...
            fh.write(f"{rlabel}\n")
            fh.write(
                f"  Rows: {len(sub):,}  |  Votes removed: {sub['score'].sum():,}\n\n"
            )
        ...
        r1p = (
            anomalies[anomalies["anomaly_rule"] == "R1-numeric_party"]
            .groupby("party")["score"]
            .agg(["sum", "count"])
        ...
        for p, row in r1p.iterrows():
            fh.write(
                f"  '{p}'  (files={int(row['count'])}, total_votes={int(row['sum']):,})\n"
            )
    ...
    print(
        f"Anomalous rows removed  : {len(anomalies):,}  ({anomalies['score'].sum():,} votes)"
    )
    print(f"Parse errors            : {len(errors)}")
    print(f"\nBreakdown by election_type × ballot_type:")
    print(
        clean.groupby(["election_type", "ballot_type"])["score"]
        .sum()
        .reset_index()
